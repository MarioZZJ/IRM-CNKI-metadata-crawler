...
def get_all_year_issues(
    session: CnkiSession, pykm: str, target_years: set[str]
) -> list[dict]:
    """获取指定年份的所有刊期信息。
    ...
    """
    # 先获取第一页，得到总年份数
    html = session.get_year_list(pykm, page_idx=0)
...
def get_papers_list(
    session: CnkiSession, pykm: str, year_issue_value: str
) -> list[dict]:
    """获取某一刊期的所有论文基础信息。

    返回: [{"title": "...", "url": "...", "authors_preview": "...", "pages": "...", "column": "..."}, ...]
    """
    html = session.get_papers(pykm, year_issue_value, page_idx=0)
